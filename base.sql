...
if exists(select * from INFORMATION_SCHEMA.TABLE_CONSTRAINTS
where CONSTRAINT_NAME='fk_ratings_rating_by_user_id')
alter table Ratings drop constraint fk_ratings_rating_by_user_id
if exists(select * from INFORMATION_SCHEMA.TABLE_CONSTRAINTS
where CONSTRAINT_NAME='fk_ratings_rating_for_user_id')
alter table Ratings drop constraint fk_ratings_rating_for_user_id
...
CREATE TABLE Ratings( 
    User_id INT NULL, 
    rating_id INT PRIMARY KEY not NULL identity, 
    rating_by_user_id INT NOT NULL, 
    rating_for_user_id INT not NULL, 
    rating_astype VARCHAR(6) not NULL, 
    rating_value INT not NULL, 
    rating_comment varCHAR(150) NULL, 

    constraint fk_ratings_rating_by_user_id foreign key (rating_by_user_id) REFERENCES clients(client_id), 
    constraint fk_ratings_rating_for_user_id foreign key (rating_for_user_id) REFERENCES tutors(tutor_id), 
    --CONSTRAINT ck_cant_rate_yourself 
) 

GO 

CREATE TABLE Rating_Astype_Lookup( 
    rating_astype VARCHAR(6) PRIMARY KEY not NULL 
) 
GO 
--adding rating values to rating table 
...
CREATE TABLE Reviews( 
    ...
    review_job_id int not NULL,
    review_rating int not null,
    review_comment varchar(150) 
) 
GO 

insert into Reviews(review_by_user, review_for_user, review_job_id, review_rating, review_comment ) values  
...
--Verify 
SELECT * from ratings 
--SELECT * from Ratings_Astype_Lookup 
SELECT * from Reviews 
